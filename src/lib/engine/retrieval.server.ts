...
async function lexicalSearch(query: string, limit: number) {
  const { data, error } = await supabaseAdmin.rpc("bm25_evidence", {
    query_text: query,
    match_count: limit,
  });
  if (error) {
...
export async function hybridRetrieve(options: {
  ...
  for (const query of options.queries) {
    const [lex, vec] = await Promise.all([
      lexicalSearch(query, 10),
      vectorSearch(query, 10),
    ]);
    lex.forEach((row) =>
      lexical.set(row.id, Math.max(lexical.get(row.id) ?? 0, row.score)),
    );
    vec.forEach((row) =>
      semantic.set(row.id, Math.max(semantic.get(row.id) ?? 0, row.similarity)),
    );
  }
  ...
  const scored: RetrievedChunk[] = (
    ...
    .map((row) => ({
      ...row,
      ...(row.documents?.title ? { document_title: row.documents.title } : {}),
    }))
    .filter(
      (chunk) =>
        targetJurisdictions.has(chunk.jurisdiction) ||
        chunk.jurisdiction === "Global",
    )
    .map((chunk) => {
      ...
      const authorityScore = Math.max(
        0,
        1 - (chunk.authority_level - 1) * 0.18,
      );
      ...
      const sourceType =
        chunk.source_type ?? chunk.sources?.source_type ?? "curated_internal";
      const sourceTypeScore = sourceType.startsWith("official")
        ? 1
        : sourceType === "recognized_institution"
          ? 0.75
          : sourceType === "secondary_reference"
            ? 0.5
            : 0.25;
      const qualityScore =
        chunk.evidence_quality === "VERIFIED"
          ? 1
          : chunk.evidence_quality === "CURATED"
            ? 0.55
            : chunk.evidence_quality === "SYNTHETIC"
              ? 0
              : 0.25;
      ...
      if (lexicalScore > 0)
        reasons.push("keyword match on the submitted terminology");
      if (semanticScore > 0)
        reasons.push("semantic similarity to the assessment query");
      if (jurisdictionMatch)
        reasons.push(`jurisdiction match (${chunk.jurisdiction})`);
      reasons.push(
        `${AUTHORITY_LABEL[chunk.authority_level] ?? "Source"} authority weighting`,
      );
      if (sourceType.startsWith("official"))
        reasons.push("official source type");
      if (chunk.evidence_quality === "CURATED")
        reasons.push(
          "curated evidence quality; source passage verification required",
        );
      ...
      return {
        ...
        fusedScore,
        rerankScore,
        why: `Retrieved and reranked by ${reasons.join(", ")}.`,
      };
    });
