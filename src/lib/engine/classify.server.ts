...
function haystack(product: Product): string {
  return [
    ...
    ...product.ingredients.flatMap((i) => [i.commonName, i.scientificName, i.plantPart]),
...
function scoreRule(rule: Rule, text: string): { score: number; hits: string[] } {
  ...
  for (const penalty of rule.penalty ?? []) {
    if (penalty.terms.some((term) => text.includes(term))) score -= penalty.weight;
  }
...
export function classifyProduct(product: Product): ClassificationOutcome {
  ...
  const missing = (key: string, value: string | undefined) => {
    ...
    return status !== "not_applicable" && (status === "not_provided" || status === "unknown" || !value?.trim());
  };

  for (const dimension of rules.dimensions as unknown as { id: string; label: string; default: string; rules: Rule[] }[]) {
    ...
    if (!winner) {
      classifications.push({
        ...
        confidence: 30,
        rationale: "The submitted information does not contain enough signals to determine this dimension. Additional product detail is required.",
        evidenceIds: [],
        review: true,
      });
    ...
    const confidence = Math.round((ambiguous ? winner.score * 0.85 : winner.score) * 100);
  ...
  if (innovation.startsWith("Novel")) ipRoutes.push("process or composition patent review");
  ipRoutes.push("trademark screening for the product name");
  if (text.includes("region") || text.includes("origin") || text.includes("geograph")) ipRoutes.push("geographical indication review");
  if (product.innovation && !innovation.startsWith("Novel")) ipRoutes.push("trade secret / confidentiality review");
  ...
  if (!product.ingredients.some((i) => i.scientificName)) missingInformation.push("Scientific (botanical) names for each ingredient");
  if (!product.ingredients.some((i) => i.plantPart)) missingInformation.push("Plant part used for each botanical ingredient");
  if (!product.ingredients.some((i) => i.sourceStatus && i.sourceStatus !== "unknown"))
    missingInformation.push("Whether each biological resource is cultivated or wild-collected");
  if (!product.ingredients.some((i) => i.geographicSource)) missingInformation.push("Geographic source and supplier record for the biological resource");
  if (missing("traditionalReference", product.traditionalReference)) missingInformation.push("Traditional text reference, where the preparation relies on one");
  if (missing("dosageForm", product.dosageForm)) missingInformation.push("Dosage form and serving size");
  if (!product.markets || product.markets.length === 0) missingInformation.push("Confirmed list of target markets");
  if (missing("existingRegistration", product.existingRegistration)) missingInformation.push("Existing licence or registration status");
  ...
  const queries = [
    ...
    `access benefit sharing biological resource ${product.ingredients.map((i) => i.scientificName).filter(Boolean).join(" ")}`,
    `trademark registration classes brand name`,
    ...(product.markets ?? [product.country]).map((market) => `${market} market category requirements for ${product.type}`),
  ].filter((q) => q.trim().length > 8);

  return { classifications, topics: [...topics], queries, missingInformation, signals };
}
