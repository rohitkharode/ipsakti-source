...
export function citationHasSupport(
  claimText: string,
  ...
  const tokens = (text: string) =>
    new Set(
      text
        ...
        ?.filter((token) => !CITATION_STOP_WORDS.has(token)) ?? [],
    );
  const claimTokens = tokens(claimText);
  const evidenceTokens = new Set(
    citedEvidence.flatMap((evidence) => [...tokens(evidence.provision)]),
  );
  return (
    claimTokens.size === 0 ||
    [...claimTokens].some((token) => evidenceTokens.has(token))
  );
}
...
export async function generateGroundedExplanation(input: {
  ...
  if (input.evidence.length === 0) {
    return {
      summary:
        "No authoritative evidence passed validation for this submission, so no grounded explanation was produced.",
      claims: [],
      uncertainties: ["Insufficient authoritative evidence."],
      generated: false,
      guardNotes: [
        "Generation skipped: safe abstention on empty evidence set.",
      ],
    };
  }
  ...
  const evidenceBlock = input.evidence
    .map(
      (e) =>
        `[${e.id}] ${e.title} — ${e.section} (${e.jurisdiction}, ${e.version}, effective ${e.effectiveDate})\n${e.provision}`,
    )
    .join("\n\n");
  const provenanceBlock = input.evidence
    .map(
      (e) =>
        `[${e.id}] quality=${e.evidenceQuality ?? "UNKNOWN"}; verification=${e.verificationStatus ?? "unknown"}; sourceType=${e.sourceType ?? "unknown"}; document=${e.documentId ?? "unknown"}; chunk=${e.provenance?.chunkId ?? "unknown"}; url=${e.url ?? "unavailable"}`,
    )
    .join("\n");
  ...
  try {
    const generationRequest = {
      ...
      user: `PRODUCT\n${input.productSummary}\n\nPRELIMINARY CLASSIFICATION\n${input.classifications
        .map(
          (c) =>
            `${c.dimension}: ${c.value} (confidence ${c.confidence}%) — ${c.rationale}`,
        )
        .join(
          "\n",
    ...
    try {
      ...
      console.warn(
        "structured grounded generation failed; retrying without response schema",
        structuredError,
      );
    ...
    return {
      summary:
        "The grounded explanation could not be generated for this analysis. The classification, evidence and routing results below were produced by the deterministic engine and are unaffected.",
      claims: [],
  ...
  const hasSemanticSupport = (claimText: string, evidenceIds: string[]) => {
    return citationHasSupport(
      claimText,
      input.evidence.filter((e) => evidenceIds.includes(e.id)),
    );
  };
  for (const claim of raw.claims ?? []) {
    const cited = (claim.supporting_evidence ?? []).filter((id) =>
      validIds.has(id),
    );
    const dropped = (claim.supporting_evidence ?? []).filter(
      (id) => !validIds.has(id),
    );
    if (dropped.length)
      guardNotes.push(
        `Removed ${dropped.length} citation(s) that do not exist in the retrieved evidence set.`,
      );
    let level = (SUPPORT_LEVELS as string[]).includes(claim.support_level)
      ? (claim.support_level as SupportLevel)
      : "insufficient_evidence";
    if (cited.length === 0) {
      level = "unsupported";
      guardNotes.push(
        "A statement without traceable evidence was downgraded to unsupported and is not presented as a finding.",
      );
      continue;
    }
    if (!hasSemanticSupport(claim.claim, cited)) {
      guardNotes.push(
        `Flagged claim without meaningful lexical alignment to its cited evidence: ${claim.claim.slice(0, 80)}.`,
      );
      continue;
    }
    if (
      /guarantee|approved|fully compliant|is patentable|definitely|no permission/i.test(
        claim.claim,
      )
    ) {
      guardNotes.push(
        "A statement using definitive legal wording was removed.",
      );
      continue;
    }
    claims.push({
      claim: claim.claim,
      supportingEvidence: cited,
      supportLevel: level,
    });
  }
  ...
  const supported = claims.filter(
    (c) =>
      c.supportLevel === "supported" ||
      c.supportLevel === "partially_supported",
  );
  if (supported.length === 0)
    guardNotes.push(
      "No claim passed the citation guard; confidence was downgraded.",
    );

  const summaryIds = (raw.summary_evidence ?? []).filter((id) =>
    validIds.has(id),
  );
  const safeSummary =
    summaryIds.length && hasSemanticSupport(raw.summary ?? "", summaryIds)
  ...
  if (summaryIds.length === 0 && raw.summary?.trim())
    guardNotes.push(
      "Replaced a summary without traceable evidence with a safe limitation.",
    );
