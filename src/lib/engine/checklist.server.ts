...
export function buildChecklist(input: ChecklistInput): Draft[] {
  const items: Draft[] = [];
  let position = 0;
  const add = (item: Omit<Draft, "position">) =>
    items.push({ ...item, position: position++ });

  const pathway = input.classifications.find(
    (c) => c.dimension === "Product pathway",
  );
  add({
  ...
  const claim = input.classifications.find(
    (c) => c.dimension === "Claim profile",
  );
  add({
  ...
  if (
    input.pathways.some(
      (p) => p.name === "Patent" && p.relevance === "Potentially relevant",
    )
  ) {
    add({
      title: "Commission a process-focused prior-art search",
      description:
        "Search patent and non-patent literature, including documented traditional knowledge, for the described process before any novelty opinion.",
      status: "pending",
      priority: "High",
      evidenceId: input.pathways.find((p) => p.name === "Patent")
        ?.evidenceIds?.[0],
    });
  }
  ...
  add({
    ...
    description:
      "Screen the proposed product and range names on the register in the relevant classes for each target market.",
    status: "pending",
    priority: "Medium",
    evidenceId: input.pathways.find((p) => p.name === "Trademark")
      ?.evidenceIds?.[0],
  });
  ...
  input.compliance
    ...
    .forEach((route) => {
      add({
        ...
        description:
          `${route.pathway}. Relevant authority: ${route.authority}. ${route.uncertainty ?? ""}`.trim(),
        status: "pending",
        priority: "Low",
  ...
  input.missingInformation.slice(0, 3).forEach((gap) => {
    add({
      ...
      description:
        "This information is required before the assessment can move beyond a screening-level result.",
      status: "pending",
      priority: "Medium",
    });
  });
  ...
  if (input.confidence < 60) {
    add({
      ...
      description:
        "Overall evidence confidence is below the threshold for a screening conclusion. Route the case to a qualified reviewer with the escalation packet.",
      status: "review",
      priority: "High",
    });
  }
